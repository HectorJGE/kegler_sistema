function initializeCreditNoteInputs() {
    ...
    getLastCreditNoteNumberByCompany();
    calcTotal();
    ...
    $('#id_credit_note_date').click(function () {
        console.log('click en fecha de emision de nota de crédito');
        $(this).select();
    });
    ...
    $("#id_create_credit_note_form").submit(function (e) {
...
function getLastCreditNoteNumberByCompany() {
    let company_id = $('#id_company').val();
    $.ajax({
        type: 'GET',
        url: url_get_last_credit_note_number.replace('9999', $('#id_company').val()),
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            if (msg !== "") {
                ...
                $('#id_credit_note_number').val(0);
                $('#id_credit_note_stamp').val(0);
            } else {
                $('#id_credit_note_number').val(data[0].credit_note_number);
                $('#id_credit_note_stamp').val(data[0].stamp_id);
            }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            /* Alerta de Error */
            Swal.fire({
                icon: 'error',
                title: 'Error...',
                text: 'Error al tratar de obtener el ultimo nro de nota de crédito de la empresa seleccionada.',
                footer: '<p>Favor póngase en contacto con el administrador del sistema.</p>'
            });
